...
impl TreeNode {
    fn new(items: &[LexerToken]) -> TreeNode {
        ...
        match next_op_pos {
            Some(pos) => {
                ...
                TreeNode { 
                    token_type,
                    left: Some(Box::new(TreeNode::new(left_items))), 
                    right: Some(Box::new(TreeNode::new(right_items))), 
                }
            },
...
impl ParseTree {
    pub fn new(lexed: &[LexerToken]) -> ParseTree {
...
pub fn evaluate_string(input: &str, vars: SharedVars) -> f32 {
    let lexed = lex(input);
    let tree: ParseTree = ParseTree::new(&lexed);
